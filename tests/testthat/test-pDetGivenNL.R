...
test_that("pDetGivenNL agrees with a brute force simulation", {
  ...
  pInt <- pDetGivenNL(nlFixed, testDetector, TL, testSL)
...
test_that("pDetGivenNL falls one decade per 10 dB under spherical spreading", {
  ...
  TL <- tlSpherical()
  p  <- pDetGivenNL(c(90, 100, 110), testDetector, TL, testSL)
...
test_that("pDetGivenNL is insensitive to bin width", {
  ...
  TL <- tlSpherical()
  expect_equal(pDetGivenNL(84, testDetector, TL, testSL, binWidth = 1),
               pDetGivenNL(84, testDetector, TL, testSL, binWidth = 0.1),
               tolerance = 5e-3)
})

test_that("pDetGivenNL falls monotonically with noise", {
  TL <- tlSpherical()
  p  <- pDetGivenNL(seq(60, 120, by = 5), testDetector, TL, testSL)
...
test_that("pDetGivenNL is relative to the truncated area", {
  ...
  TL <- tlSpherical()
  expect_gt(pDetGivenNL(84, testDetector, TL, testSL,
                        truncationDistance = 1e5),
            pDetGivenNL(84, testDetector, TL, testSL))
})

test_that("pDetGivenNL accepts a detFun object as well as a plain function", {
  TL      <- tlSpherical()
  snrInfo <- make_snr_data()
  p <- pDetGivenNL(84, fitDetFun(snrInfo, modelType = "glm"), TL, testSL)
  expect_true(p >= 0 && p <= 1)
})
