...
cde <- function (p,season){
  ...
  # ch %>% 
  #   group_by(season) %>%
  #   dplyr::summarize(false.pos.num = sum(detect_table2 & !detect_table1),
  #                    true.pos.num = sum(detect_table2 & detect_table1),
  ...
  write.csv(result, file = p$densityResultsFile, row.names=F)
  return(result)
}
