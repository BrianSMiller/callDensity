...
|---------------|---------------|---------------|---------------|---------------|
| Detector 1 | Reliable and good detector, meant to emulate a human analyst. | Lower | Lower | Lower |
| Detector 2 | Automated detector, meant to emulate a signal processing algorithm. | Higher | Higher | Higher |
...
Attempt to apply an adjudicated capture-recapture model using VGAM package.
...
preds.gam= predict(snrDetFun.gam, type='response',newdata=snrs)
preds.scam=predict(snrDetFun.scam,type='response',newdata=snrs)
...
plot(snrs$SNR, xlim= c(-5,15),  ylim= c(0,1),
     xlab="SNR", ylab = "P(Detection)", type='n')
grid(nx = NULL, ny = NULL, lty = 2, col = "gray", lwd = 2)
lines(snrs$SNR,preds.glm,col=cols(7)[4],lwd=2)
lines(snrs$SNR,preds.gam,col=cols(7)[3],lwd=2)
lines(snrs$SNR,preds.scam,col=cols(7)[2],lwd=2)
...
lines(snrs$SNR,preds.vglm0,col=cols(7)[1],lwd=2)    # vglm 1-p(all zero)
