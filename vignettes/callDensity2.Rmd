...
# Validate that we've subsetted the data sensibly by time.
...
ggplot(subsampleDet2, aes(x=datetime, weights=detect_table) )+
  # geom_point(size=1,alpha=0.1)
  geom_histogram(breaks=seq(from=minDate,to=maxDate,by='1 day'))
...
```{r GAM p.det, eval=FALSE, message=FALSE, warning=FALSE, include=FALSE}
...
```{r SCAM p.det, eval=FALSE, message=FALSE, warning=FALSE, include=FALSE}
...
NLind<- NLsamp
NLind$mean <- mean(subset(capHistTab$noiseRMSdB1,capHistTab$subset1),na.rm=T)
NLind$sd <- sd(subset(capHistTab$noiseRMSdB1,capHistTab$subset1),na.rm=T)
...
# 'Independent' estimate of NL 
NLadj<- NLsamp
NLadj$mean <- mean(subset(capHistTab$noiseRMSdB1,capHistTab$subset1),na.rm=T)
NLadj$sd <- sd(subset(capHistTab$noiseRMSdB1,capHistTab$subset1),na.rm=T)
...
legend( "bottomright", 
        legend =c("Detection function TRUE",
                  ...
                  "One minus probability of all zeros (VGLM)"
                  ),
