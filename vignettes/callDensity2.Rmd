...
n = 5e6; # number of simulated calls (regardless of whether detected or not); 
...
glmTime1 <- system.time(results.glm <- cde(Nc=Nc, capHistTab = capHistTab, snrDetFun = snrDetFun.glm,
...
glmTime2 <- system.time(results.NLind <- cde(Nc=Nc, capHistTab = capHistTab, snrDetFun = snrDetFun.glm,
...
vglmTime <- system.time(results.vglm <- cde(Nc=Nc, capHistTab = ch, snrDetFun = snrDetFun.vglm,
                   SL = SL, TL = TL, A = A, modelType = 'vglm'))

cat('Computational costs:\n\n')
print(rbind(glmTime1,glmTime2,vglmTime))
