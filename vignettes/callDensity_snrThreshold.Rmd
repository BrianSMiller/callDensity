---
title: "SNR truncation: fixing capture-recapture's blind spot"
output: 
  ...
  rmarkdown::html_vignette:
    vignette: >
      ...
      %\VignetteIndexEntry{SNR truncation: fixing capture-recapture's blind spot}
...
# averaging), so that's built explicitly here for both tables.
capHistTab$SNR  <- rowMeans(capHistTab[,  c("snr_observer1", "snr_observer2")], na.rm = TRUE)
capHistTabt$SNR <- rowMeans(capHistTabt[, c("snr_observer1", "snr_observer2")], na.rm = TRUE)
...
Capture-recapture with two fallible observers means neither `detect_observer1` nor `detect_observer2` is ground truth on its own. `cde()` accepts `groundTruthCol`/`observerCol` directly, so whichever column actually holds the adjudicated truth -- here, the simulated `groundTruth` -- can be pointed to without renaming anything in the table itself. Skipping this does not error -- it silently computes a false discovery rate for one observer against the other, which is not a meaningful quantity and will not match the truth.

```{r cde.vglm, warning=FALSE, message=FALSE}
library(VGAM)

adjudicated <- subset(capHistTab,
                capHistTab$groundTruth &
                (capHistTab$detect_observer1 | capHistTab$detect_observer2))
adjudicatedt <- subset(capHistTabt,
                capHistTabt$groundTruth &
                (capHistTabt$detect_observer1 | capHistTabt$detect_observer2))
adjudicatedt$SNR <- rowMeans(
  subset(adjudicatedt, select = c("snr_observer1", "snr_observer2")), na.rm = TRUE)
...
# calculation. signalCol/noiseCol point at each observer's own suffixed
# signal/noise columns, which chtToSNRinfo() averages per event.
...
runCell <- function(theta, det1scale, seed, n = 5e4, outerloop = 10) {
  ...
  # needed below both for the truncation filter and for cde()'s own
  # falseDiscoveryRate() call.
  ch$SNR <- rowMeans(ch[, c("snr_observer1", "snr_observer2")], na.rm = TRUE)
  ...
  truePaTrunc <- mean(ch$p_det_observer2 * (ch$snr_observer2 >= theta), na.rm = TRUE)
  ...
  # groundTruthCol/observerCol point cde() at the simulated ground truth
  # (groundTruth) directly -- the synthetic equivalent of a judge's
  ...
  # signalCol/noiseCol point at each observer's own suffixed signal/noise
  # columns, which chtToSNRinfo() averages per event.
