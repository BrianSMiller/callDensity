...
$CV.D_c$ falls with detector count -- confirmed as a general pattern across several random detector-parameter draws during development, not just the one run shown here. The reason is the same one from the two-detector case, just compounding: each additional detector adds more events to the union's adjudicated positive set, which is what the false discovery rate's precision depends on directly. Ten randomly-drawn commodity detectors pooled this way substantially outprecision two decent ones -- an argument for breadth over quality when detectors are genuinely cheap to run in parallel.
