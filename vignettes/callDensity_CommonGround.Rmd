---
title: "Imperfect ground truth: capture-recapture (Common Ground) as the fix"
output: 
  ...
  rmarkdown::html_vignette:
    vignette: >
      ...
      %\VignetteIndexEntry{Imperfect ground truth: capture-recapture (Common Ground) as the fix}
...
Code from the callDensity package used to simulate sonar eq parameters and to simulate TL: `simCallAcoustics<-` 
<details>
```{r, code=simCallAcoustics, eval=FALSE}
```
...
Code used to simulate the detection process:`simulateDetector <- ` <details>
...
```
 ...
 Code used to implement subsampling and plot to validate that we've subsetted the data sensibly by time: `subsampleSimInTime <- ` <details>
...
Code used to merge simulated detections into a capture history table: `simsTocaptureHistoryTable <- `<details>
...
The callDensity package provides a convenience function fitDetFun to fit GLM, GAM, and SCAM models: `fitDetFun <- `<details>

```{r, code=fitDetFun, eval=FALSE}
```
</details>
...
CallDensity package code for correcting NL distribution bias that occurs due to higher probability of detecting calls when NL is low. See  `vignette("callDensity_noiseLevels")`. Function code: `nlFromSnrInfo <- `<details>
```{r, code=nlFromDetections, eval=FALSE}
```
</details>
...
`groundTruth`). we want to include in our CH table all predicted positive
...
#groundTruth/detect_observer2, on a capHistTab restricted to rows flagged by
...
summary(adjudicated$group_observer1)
summary(adjudicated$group_observer2)
...
fp2 = sum(capHistTab$detect_observer2==TRUE & capHistTab$groundTruth==FALSE,
...
```{r compare snr detection functions, warning=FALSE, fig.height=4}
...
truth <- data.frame(
  SNR   = c(simDet2$snr,      capHistTab$snr_observer1),
  p_det = c(simDet2$p_det,    capHistTab$p_det_observer1),
