...
epoch <- "1970-01-01"
sim$datetime <- as.POSIXct(pmax(runif(n, minDate, maxDate),
                                runif(n, minDate, maxDate) ), 
                           origin = epoch, tz="UTC")
...
Distances are the magnitude of the x & y location. 
...
### a) Source Level (SL)
Assume source level, SL, is that of Antarctic blue whale song (@Castro2024). 
...
```{r plot TL, include=FALSE, eval=FALSE}

# NOT RUN & NOT INCLUDED IN RMARKDOWN 

# Plot the transmission loss model. Such a plot isn't usually needed for very
# simple models, such as spherical spreading. But if using more complex models
# it can be useful to visualise them to check their validity/plausibility.
...
ggplot(data=sim, aes(x=x/1e3, y=y/1e3, colour=detected_observer1))+
  geom_point(alpha=0.1,size=0.1)+
  coord_equal()+
  xlab("X location (km)")+
  ylab("Y location (km)")
...
simWithFP <- rbind(sim,fp) # Make sure not to confuse simWithFP and sim. 

# Nc total number of detected calls, including both true and false positive
Nc <- sum(simWithFP$detected_observer1)
...
## 9) Subsample data for detector characterisation
...
Table showing how the the results of each call density estimate compare to the true values.
The only difference between the three estimates is type of model used to fit the SNR-detection function (glm, gam, scam). 
...
results<-rbind( results, results.glm, results.gam, results.scam)
